...
export default function Page() {
  ...
  return (
    <>
      <section className="min-h-[150vh] bg-pink"></section>
      <section ref={ref} className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        {aboutme.map((el, ind) => (
          <div key={ind} className="col-span-1 col-start-1 text-2xl lg:min-h-[350px]">
            {el.body}
          </div>
        ))}
        {windowWidth >= 1024 &&
          aboutme.map((el, ind) => (
            <div
              key={ind}
              className="col-span-1 col-start-2 row-start-1 row-span-6 bg-blue"
              style={{ gridRowStart: ind + 1, gridRowEnd: aboutme.length + 1 }}>
              <div key={ind} className="sticky top-0 flex justify-center">
      ...
      <section className="min-h-[100vh] bg-violet">
